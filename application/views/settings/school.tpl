            ...
            <div class="col-lg-12 col-md-12 col-xs-12">
                <div class="box">
                    <div class="box-body">
                        <div class="col-md-12">
                            <div class="col-md-3">
                                <div class="form-group" id="school_box">
                                    <label>School Name <span class="text-red">*</span></label>
                                    <input type="text" name="school_name" id="school_name" class="form-control" placeholder="Please enter School name"/>
                                    <input type="hidden" name="school_id" id="school_id" value="0"/>
                                    <label id="school_error_msg"></label>
                                </div>
                            </div>
                            <div class="col-md-1">
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click','#save',function(){
            ...
            if(checkBlank('school_box','school_error_msg','Required..', school_name, 'school_name', '')){
                return false;
            }
            var control  = {
                request_id : generateUUId(),
            ...
            request = JSON.stringify(request);
            var url = "{$smarty.const.API_URL}school/add";
            $.ajax({
                method: "POST",
                url: url,
                async: true,
        ...
        $(document).on('click','.active_deactive',function(){
            ...
            request = JSON.stringify(request);
            var url = "{$smarty.const.API_URL}school/delete";
            $.ajax({
                method: "POST",
                url: url,
                async: true,
        ...
        $(document).on('click','.edit',function(){
            ...
            $('#school_name').val($('#school_'+school_id).text());
            $('#school').focus();
        });
    });
</script>
