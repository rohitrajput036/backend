...
class Cast_categoty extends REST_Controller {
    ...
    public function add_post(){
        ...
        try{
            ...
            if (!empty($request)) {
                ...
                keyExist(['cast_categoty_id','cast_name', 'short_code', 'sort_order'], $request->data);
                ...
                $this->cast_categoty_model->cast_categoty_id = $request->data->cast_categoty_id;
                $this->cast_categoty_model->cast_name = $request->data->cast_name;
                $this->cast_categoty_model->short_code = $request->data->short_code;
                $this->cast_categoty_model->sort_order = $request->data->sort_order;
                $this->cast_categoty_model->is_active = 1;
                $this->cast_categoty_model->created_by = $this->cast_categoty_model->updated_by = (isset($request->data->login_id) && $request->data->login_id > 0) ? $request->data->login_id : 0;
                $message = 'Your Cast add successfully';
                if(isset($request->data->cast_categoty_id) && $request->data->cast_categoty_id > 0){
                    $this->cast_categoty_model->cast_categoty_id = $request->data->cast_categoty_id;
                    $this->cast_categoty_model->update();
                    $message = 'Cast update successfully!';
                }else{
                    if(!$this->cast_categoty_model->check()){
                        $this->cast_categoty_model->add();
                    }else{
    ...
    public function delete(){
        ...
        try{
            ...
            if (!empty($request)) {
                keyExist(['control', 'data'],$request);
                keyExist(['request_id', 'source', 'request_time'],$request->control);
                keyExist(['cast_categoty_id', 'is_active'], $request->data);
                checkBlank(['request_id' => $request->control->request_id,'source' => $request->control->source,'request_time' => $request->control->request_time]);
                checkBlank(['cast_categoty_id' => $request->data->cast_categoty_id,'is_active' => $request->data->is_active]);
                $this->cast_categoty_model->cast_categoty_id = $request->data->cast_categoty_id;
                $this->cast_categoty_model->is_active = $request->data->is_active;
                $this->cast_categoty_model->created_by = $this->cast_categoty_model->updated_by = (isset($request->data->login_id) && $request->data->login_id > 0) ? $request->data->login_id : 0; 
                $this->cast_categoty_model->delete();
                $message = 'Cast update successfully';
                if($request->data->is_active == 1){
                    $message = 'Cast ID activate successfully';
                }else if($request->data->is_active == 2){
                    $message = 'Cast ID dactivate successfully';
                }else if($request->data->is_active == 3){
                    $message = 'Cast ID delete successfully';
                }
            }else{
                throw new Exception('Invalid request',400);
            }
    ...
    function get_post(){
        ...
        try {
            ...
            if (!empty($request)) {
                ...
                $data = $this->cast_categoty_model->get($request->data->for_table);
            } else {
