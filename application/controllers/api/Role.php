    ...
    function add_post(){
        ...
        try {
            ...
            if (!empty($request)) {
                ...
                keyExist(['role_id','role'],$request->data);
                checkBlank(['request_id' => $request->control->request_id,'source' => $request->control->source,'request_time' => $request->control->request_time]);
                checkBlank(['role' => $request->data->role]);
                $this->role_model->role = $request->data->role;
                $this->role_model->is_active = 1;
                ...
                if(isset($request->data->role_id) && $request->data->role_id > 0){
                    $this->role_model->role_id = $request->data->role_id;
                    $this->role_model->update();
                    $message = 'role update successfully!';
                }else{
                    if(!$this->role_model->check()){
                        $this->role_model->add();
                    }else{
                        throw new Exception('role aready exists!',400);
                    }
                }
            }else{
