    ...
    function add_post(){
        ...
        try {
            ...
            if (!empty($request)) {
                ...
                keyExist(['country_id','country_name'],$request->data);
                checkBlank(['request_id' => $request->control->request_id, 'source' => $request->control->source, 'request_time' => $request->control->request_time]);
                checkBlank(['country_name' => $request->data->country_name]);
                $this->country_model->country_name = $request->data->country_name;
                $this->country_model->is_active = 1;
                ...
                if(isset($request->data->country_id) && $request->data->country_id > 0){
                    $this->country_model->country_id = $request->data->country_id;
                    $this->country_model->update();
                    $message = 'Country update successfully!';
                }else{
                    if(!$this->country_model->check()){
                        $this->country_model->add();
                    }else{
                        throw new Exception('This Country aready exists!',400);
                    }
                }
            }else{
    ...
    function delete_post(){
        ...
        try {
            ...
            if (!empty($request)) {
                keyExist(['control','data'],$request);
                keyExist(['request_id','source','request_time'],$request->control);
                keyExist(['country_id','is_active'],$request->data);
                checkBlank(['request_id' => $request->control->request_id,'source' => $request->control->source,'request_time' => $request->control->request_time]);
                checkBlank(['country_id' => $request->data->country_id,'is_active' => $request->data->is_active]);
                $this->country_model->country_id = $request->data->country_id;
                ...
                if($request->data->is_active == 1){
                    $message = 'Country ID activate successfully';
                }else if($request->data->is_active == 2){
                    $message = 'Country ID dactivate successfully';
                }else if($request->data->is_active == 3){
                    $message = 'Country ID delete successfully';
                }
            }else{
                throw new Exception('Invalid request',400);
            }
    ...
    function get_post(){
        ...
        try {
            ...
            if (!empty($request)) {
                ...
                $data = $this->country_model->get($request->data->for_table);
            } else {
